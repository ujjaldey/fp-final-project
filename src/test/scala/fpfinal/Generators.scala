...
trait Generators {
  ...
  /**
   * TODONE #3a: implement an arbitrary of Person.
   *
  ...
  /**
   * TODONE #3b: Use the provided arbitraries and the Expense.unsafeCreate method
  ...
  implicit def expenseArb(implicit
    ...
    for {
      ...
      participants <- Gen.nonEmptyListOf(person)
  ...
  /**
   * TODONE #3c: implement an arbitrary of PersonOp[A].
   *
